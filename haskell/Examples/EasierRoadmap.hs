...
import Prelude ((.), ($), return, undefined, error, IO)
...
easierRoadmapProg2' =
    fromJust (determine (disintegrate easierRoadmapProg1))
...
simplify :: (ABT Term abt) => abt '[] a -> abt '[] a
...
mh prop target =
    let_ (fromJust . determine $ density target) $ \mu ->
    lam $ \old ->
        prop old >>= \new ->
        dirac $ pair new (mu `app` new / mu `app` old)


sq :: (ABT Term abt, HSemiring_ a) => abt '[] a -> abt '[] a
...
runSample :: MWC.GenIO
          ...
          -> IO (Maybe (Value a))
runSample g prog =
  case runEvaluate prog of 
    VMeasure m -> do
      Just (s, _) <- m (VProb 1) g
      return (Just s)

runEasierRoadmapProg4, runEasierRoadmapProg4'
    :: IO (Maybe (Value ('HArray (HPair 'HProb 'HProb))))

...
t14 =
    bern (prob_ (3/5)) >>= \b ->
    if_ b t13 (bern (prob_ (2/7)) >>= \b' ->
        if_ b' (uniform (real_ 10) (real_ 12)) (uniform (real_ 14) (real_ 16)))
t14' = superpose 
    [ (fromRational (9/25), dirac (real_ 37))
    , (fromRational (6/25), dirac (real_ 42))
    , (fromRational (4/35), uniform (real_ 10) (real_ 12))
...
t23 =
    bern half >>= \a ->
    bern (if_ a (prob_ (9/10)) (prob_ (1/10))) >>= \b ->
    bern (if_ a (prob_ (9/10)) (prob_ (1/10))) >>= \c ->
    dirac (pair b c)
t23' = superpose
...
--t44Add  = lam $ \x -> lam $ \y -> weight (unsafeProb (x * x) + unsafeProb (y * y))
--t44Add' = lam $ \x -> lam $ \y -> weight (unsafeProb (x ** 2 + y ** 2))
--t44Mul  = lam $ \x -> lam $ \y -> weight (unsafeProb (x * x * y * y))
--t44Mul' = lam $ \x -> lam $ \y -> weight (unsafeProb (x ** 2) * unsafeProb (y ** 2))
...
--    if_ (t <= zero) (superpose []) $
--    if_ (t <= one) (weight (unsafeProb t)) $
--    if_ (t <= 2) (weight (unsafeProb (2 + t * negate one))) $
--    superpose []
...
--t57' = lam $ \t -> 
--    if_ (t < one && zero < t) (weight 2) (dirac unit)
...
--      if_ (t/x <= 2) (weight (unsafeProb (2-t/x))) $
--      superpose []
...
--      if_ (t/x <= 2) (weight (unsafeProb (2-t/x) / unsafeProb x)) $
--      superpose []

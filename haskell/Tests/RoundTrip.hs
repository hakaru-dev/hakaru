...
testMeasureUnit = test [
    ...
    --TODO"t64"     ~: testSStriv [t64,t64'] t64'',
...
half = (fromRational 1) / (fromRational 2)

third :: (ABT Term abt, HFractional_ a) => abt '[] a
third = (fromRational 1) / (fromRational 3)
...
--     $ if_ (t < one) (weight (unsafeProb (log t + one)))
--     $ if_ (t < one + exp (negate one)) (dirac unit)
--     $ if_ (t < 2) (weight (unsafeProb (negate (log (t - one)))))
--     $ superpose []
...
t68 =
    ...
    withWeight (exp (negate (x2 - x3) * (x2 - x3)
                     * recip (fromProb ((prob_ 2) * exp (log x4 * (prob_ 2)))))
              * recip x4
              * recip (exp (log ((prob_ 2) * pi) * half)))
             (withWeight (exp (-(x1 - x3) * (x1 - x3)
                             * recip (fromProb ((prob_ 2) * exp (log x5 * (prob_ 2)))))
                      * recip x5
                      * recip (exp (log ((prob_ 2) * pi) * half)))
                     (withWeight (exp (-x3 * x3
                                     * recip (fromProb ((prob_ 2) * exp (log x4 * (prob_ 2)))))
                              * recip x4
                              * recip (exp (log ((prob_ 2) * pi) * half)))
                             (dirac x2)))
          ...
          (lam $ \x4 ->
            ...
            + one
              * (lam $ \x5 ->
                 (lam $ \x6 ->
                  zero
                  + exp (-(x2 - zero) * (x2 - zero) / fromProb ((prob_ 2) * exp (log (prob_ 5) * (prob_ 2))))
                    / (real_ 5)
                    / exp (log ((prob_ 2) * pi) * half)
           ...
           (lam $ \x6 ->
            ...
            + one
              * (lam $ \x7 ->
                 (lam $ \x8 ->
                  zero
                  + exp (-(x4 - x3) * (x4 - x3) / fromProb ((prob_ 2) * exp (log one * (prob_ 2))))
                    / one
                    / exp (log ((prob_ 2) * pi) * half)
...
testKernel2 =
    lam $ \x2 ->
    normal x2 one >>= \x3 ->
    let q = exp(-(real_ 1)/(real_ 50)*(x3-x2)*(x3+x2)) in
        ...
        [(one,
            ...
            superpose
                [(one,
                    ...
                    withWeight
                        ( exp (-(x3 - x6) * (x3 - x6)
                            / (fromProb ((prob_ 2) * exp (log (unsafeProb x5) * (prob_ 2)))))
                        / unsafeProb x5
                        / (exp (log ((prob_ 2) * pi) * half))) $
                    withWeight one $
                    lebesgue >>= \x7 ->
                    withWeight
                        ( exp (-(x6 - x7) * (x6 - x7)
                            / (fromProb ((prob_ 2) * exp (log (unsafeProb x4) * (prob_ 2)))))
                        / (unsafeProb x4)
                        / (exp (log ((prob_ 2) * pi) * half))) $
                    withWeight
                        ( exp (-(x2 - x7) * (x2 - x7)
                    ...
                    withWeight
                        ...
                        / unsafeProb x4
                        / (exp (log ((prob_ 2) * pi) * half))) $
                    withWeight (recip (prob_ 3)) $
                    superpose
                        [(one,
                            if_ (x5 < (real_ 4))
                                (if_ (one < x5)
                                    (weight (recip (prob_ 5)) $
                                    superpose
                                        [(one,
                                            if_ (x4 < (real_ 8))
                                                (if_ ((real_ 3) < x4)
                                             ...
                                             let_ (recip pi
                                                   ...
                                                   * exp (log (unsafeProb (exp (log (fromProb x4)
                                                                             ...
                                                                             + exp (log (fromProb x5)
                                                                                    * (real_ 4))))
                                                           * (negate half))
                                                   * (real_ 1)/(real_ 10)) $ \x32 ->
                                             let_ (let_ (recip (prob_ 3)) $ \x33 ->
                                                   let_ (let_ one $ \x34 ->
                                                         let_ (if_ (fromProb x5 < (real_ 4))
                                                                   (if_ (one < fromProb x5)
                                                                        (let_ (recip (prob_ 5)) $ \x35 ->
                                                                         let_ (let_ one $ \x36 ->
...
seismicFalseDetection x0 =
    ...
    uniform (-(real_ 23)/(real_ 500) * (real_ 180) + (real_ 107)/(real_ 10)) (-(real_ 23)/(real_ 500) * zero + (real_ 107)/(real_ 10)) >>= \x31 ->
    (
        normal_0_1 >>= \x32 ->
        normal_0_1 >>= \x33 ->

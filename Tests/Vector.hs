...
unreduced, reduced
    ...
    => repr (HFun HInt (HMeasure HInt))
unreduced = lam $ \n -> dirac (sumV (vector 4 (n+)))
reduced = lam $ \n -> dirac (n * 4 + 6)

-- Test unrolling short product measures
rolled, unrolled :: (Mochastic repr, Integrate repr) => repr (HMeasure HProb)
...
count, unity :: (Mochastic repr) => repr (HMeasure HInt)
...
norm_nox_s,norm_nox_s' :: (Mochastic repr) => repr (HArray (HMeasure HReal))
...
joint
    ...
    => repr (HArray HProb)
    -> repr (HMeasure (HPair HInt (HArray HProb)))
...
plateDirac,plateDirac' :: (Mochastic repr) => repr (HMeasure (HArray HReal))
plateDirac = plate (vector 10 (dirac . (1+) . fromInt))
plateDirac' = dirac (vector 10 ((1+) . fromInt))

t75 :: Mochastic repr => repr (HMeasure (HArray HReal))
t75 = poisson 8 `bind` \n ->
      plate $ vector n (\ _ ->
                        normal 0 1)

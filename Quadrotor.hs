...
prog_quadCopter :: Mixture (Double, Double, Double, Double) -> 
                   Double -> 
                   Double ->
                   Double -> 
...
-- helper functions
compareMix (_, v1) (_, v2) = if (v1 < v2) then GT
  else LT
...
convertTime (Lebesgue t) = case fromDynamic t of
  Just t -> t
  Nothing -> error "Not conditioned data"
convertTime (Discrete t) = case fromDynamic t of
  Just t -> t
  Nothing -> error "Not conditioned data"
...
convertC (Left _) = [[]]
convertC (Right s) = map (\x0 -> map (\x1 ->Lebesgue (toDyn(read x1::Double))) x0) (s)
...
convertTable table = foldl (\acc x -> acc ++ [convertCSV x (head x)]) [] table 

convertCSV :: [String] -> String -> String
convertCSV list firstElement = foldl (\acc x -> if (x == firstElement) then acc ++ x  
  else acc ++ "," ++ x) "" list 
...
mergeGPS time_g time_r results n list = 
  ...
  (if (time_r !! n) `elem` time_g  then
   [[show (time_r !! n), show ((results !! n) !! 1), show ((results !! n) !! 2)]] ++ list else list)

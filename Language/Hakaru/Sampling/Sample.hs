...
--                                                    2015.10.18
-- |
-- Module      :  Language.Hakaru.Sampling.Sample
    ...
    superpose pms@((_, Sample m) : _) = Sample (\p g -> do
        ...
        if not (y > (0::Double)) then return Nothing else do
            ...
            case [ m1 | (v,(_,m1)) <- zip (scanl1 (+) ys) pms, u <= v ] of
                Sample m2 : _ -> (m2 $! p * x * LF.logFloat y) g
...
instance Integrate (Sample m) where -- just for kicks -- inaccurate
    integrate (Sample lo) (Sample hi)
        ...
        where
        int method f = Sample $ LF.logFloat $ TS.result $ last
                       $ method $ LF.fromLogFloat . unSample . f . Sample
    summate (Sample lo) (Sample hi) f =
        integrate (Sample lo') (Sample hi') f'
        where
        lo' | isInfinite lo = lo
...
runSample m = do
    g <- MWC.createSystemRandom -- "This is a somewhat expensive function, and is intended to be called only occasionally (e.g. once per thread)."
    s <- unSample m 1 g
    return (fmap fst s)
